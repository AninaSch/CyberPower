...
data_norm1 = normalise_ci(surveillance,c(2:7),c("POS","POS","POS","POS","POS","POS"),
                         method=1,z.mean=50, z.std=10) #   method=1,z.mean=50, z.std=10)
...
# CPI_norms
data_norm7 = normalise_ci(norms,c(2:6),c("POS","POS","NEG", "POS", "POS"),
                          method=1,z.mean=50, z.std=10) #   method=1,z.mean=50, z.std=10)
...
CPI_cybernorms <- CPI_norm7 %>% 
  gather(key, val, laws:tech_export)%>%
  group_by(country) %>%
  mutate(
    mean_norm = mean(val,  na.rm = TRUE)
  ) %>%
...
CPI_scores = data.frame(CPI, score_surveillance = CPI_surveillance$mean_surveillance,
