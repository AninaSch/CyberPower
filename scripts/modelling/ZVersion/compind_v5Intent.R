...
# CPI_manipulation
data_norm3 = normalise_ci(manipulation,c(2:7),c("POS","POS","POS","POS","POS","POS","POS"),
                          method=1,z.mean=50, z.std=20) #   method=1,z.mean=50, z.std=10)
...
# CPI_disruption
data_norm6 = normalise_ci(disruption,c(2:6),c("POS","POS","POS", "POS", "POS"),
                          method=1,z.mean=50, z.std=20) #   method=1,z.mean=50, z.std=10)
...
CPI_disruption <- CPI_norm6 %>% 
  gather(key, val, attack_disruption:cyber_command)%>%
  group_by(country) %>%
  mutate(
    mean_norm = mean(val,  na.rm = TRUE)
  ) %>%
...
CPI_scores_cap <- CPI_scores_cap %>%
  group_by(country) %>%
  mutate(score_capabilities = mean(c(score_surveillance, score_defense, score_manipulation, score_intelligence,
                                  score_commercial, score_disruption, score_cybernorms), na.rm = TRUE ))

CPI_scores <- CPI_scores_cap %>%
  group_by(country) %>%
  mutate(capint_surveillance = (score_surveillance*intent_surveillance)/100,
         capint_defense = (score_defense*intent_defence)/100,
         capint_manipulation = (score_manipulation*intent_control)/100,
         capint_intelligence = (score_intelligence*intent_intelligence)/100,
         capint_commercial = (score_commercial*intent_commercial)/100,
         capint_disruption = (score_disruption*intent_offesive)/100,
         capint_norms = (score_cybernorms*intent_norms)/100
         ) %>%
  mutate(score_capint = mean(c(capint_surveillance, capint_defense, capint_manipulation, 
                                  capint_intelligence, capint_commercial, capint_disruption, capint_norms), na.rm = TRUE))
