...
CPI_scores <- readRDS("../../data/data_for_modelling/CPI_scores_beta.rds") 
...
p_avg <- ggplot(data = average, aes(x = country, y = average)) +
  ...
  xlab("") +
  ylab("Capability Score") +
  ggtitle("Capability Index") +
  coord_flip() +
  crimson_theme() +
  scale_y_continuous(breaks = seq(0, 70, by = 10), limits = c(0,71), expand = c(0,0)) # +
# theme(plot.title.position  = "plot") # left align title
# theme(plot.title  = element_text(hjust = -0.45, vjust=2.12))

surveillance <- data2 %>% select(country, score_surveillance) %>% mutate(country = fct_reorder(country, score_surveillance)) %>% arrange(-score_surveillance)
p_surveillance <- ggplot(data = surveillance, aes(x = country, y = score_surveillance)) +
  geom_point(size = size_dot, color = "#ed1b34") +
  geom_segment(aes(xend = country, y = 0, yend = score_surveillance)) +
  xlab("") +
  ylab("Capability Score") +
...
defense <- data2 %>% select(country, score_defense) %>% mutate(country = fct_reorder(country, score_defense)) %>% arrange(-score_defense)
p_defense <- ggplot(data = defense, aes(x = country, y = score_defense)) +
  geom_point(size = size_dot, color = "#000000") +
  geom_segment(aes(xend = country, y = 0, yend = score_defense)) +
  xlab("") +
  ylab("Capability Score") +
  ggtitle("Defense") +
  coord_flip() +
...
control <- data2 %>% select(country, score_control) %>% mutate(country = fct_reorder(country, score_control)) %>% arrange(-score_control)
p_control <- ggplot(data = control, aes(x = country, y = score_control)) +
  geom_point(size = size_dot, color = "#4db848") +
  geom_segment(aes(xend = country, y = 0, yend = score_control)) +
  xlab("") +
  ylab("Capability Score") +
...
intelligence <- data2 %>% select(country, score_intelligence) %>% mutate(country = fct_reorder(country, score_intelligence)) %>% arrange(-score_intelligence)
p_intelligence <- ggplot(data = intelligence, aes(x = country, y = score_intelligence)) +
  geom_point(size = size_dot, color = "#4e88c7") +
  geom_segment(aes(xend = country, y = 0, yend = score_intelligence)) +
  xlab("") +
  ylab("Capability Score") +
...
commerce <- data2 %>% select(country, score_commercial) %>% mutate(country = fct_reorder(country, score_commercial)) %>% arrange(-score_commercial)
p_commerce <- ggplot(data = commerce, aes(x = country, y = score_commercial)) +
  geom_point(size = size_dot, color = "#fcb315") +
  geom_segment(aes(xend = country, y = 0, yend = score_commercial)) +
  xlab("") +
  ylab("Capability Score") +
  ggtitle("Commerce") +
  coord_flip() +
...
offense <- data2 %>% select(country, score_offense) %>% mutate(country = fct_reorder(country, score_offense)) %>% arrange(-score_offense)
p_offense <- ggplot(data = offense, aes(x = country, y = score_offense)) +
  geom_point(size = size_dot, color = "#00aaad") +
  geom_segment(aes(xend = country, y = 0, yend = score_offense)) +
  xlab("") +
  ylab("Capability Score") +
  ggtitle("Offense") +
  coord_flip() +
...
norms <- data2 %>% select(country, score_norms) %>% mutate(country = fct_reorder(country, score_norms)) %>% arrange(-score_norms)
p_norms <- ggplot(data = norms, aes(x = country, y = score_norms)) +
  geom_point(size = size_dot, color = "#946eb7") +
  geom_segment(aes(xend = country, y = 0, yend = score_norms)) +
  xlab("") +
  ylab("Capability Score") +
  ggtitle("Norms") +
  coord_flip() +
